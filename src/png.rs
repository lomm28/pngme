...
fn compare_vectors(vec_a: &[u8], vec_b: &[u8]) -> bool {
    let a_set: HashSet<_> = vec_a.iter().copied().collect();

    vec_b.iter().all(|item| a_set.contains(item))
}
...
impl Png {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks()
            .into_iter()
            .find(|value| value.chunk_type().to_string() == chunk_type)
    }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let chunk_by_type = self.chunk_by_type(chunk_type).unwrap();
        let chunk_copy = chunk_by_type.clone();

        if let Some(index) = self.chunks
            .iter()
            .position(|chunk| compare_vectors(&chunk_by_type.as_bytes(), &chunk.as_bytes())) {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        let (header_slice, png_slice) = bytes.split_at(8);
        let has_correct_header = compare_vectors(&header_slice, &Png::STANDARD_HEADER);

        if !has_correct_header {
            Err("file has invalid header")?;
        }

        let mut png_bytes: &[u8] = png_slice;
        let mut chunks: Vec<Chunk> = Vec::new();

        while png_bytes.len() > 0 {
            let mut length_buffer = [0; 4];
            let mut reader = BufReader::new(&*png_bytes);

            reader.read_exact(&mut length_buffer)?;
            let first_chunk_length = u32::from_be_bytes(length_buffer);

            let (first_chunk, other_chunks) = png_bytes.split_at((first_chunk_length + 12).try_into().unwrap());
            let chunk = Chunk::try_from(first_chunk.as_ref())?;

            chunks.push(chunk);
            png_bytes = other_chunks;
        }
